...
from multi_objective.utils import num_parameters, RunningMean, reference_points
...
class COSMOS2Method(BaseMethod):
    ...
    def __init__(self, objectives, model, cfg):
        ...
        self.loss_mins = torch.tensor([.35, .35]).to(self.device)
        ...
        self.steps = 0
        self.sample = reference_points(n, self.K, min=[0.2, 0.2])
        self.lagrangian = [torch.nn.parameter.Parameter(torch.zeros(self.K), requires_grad=False).cuda() for _ in range(len(self.sample))]
    ...
    def step(self, batch):
        ...
        for j, g_j in enumerate(g_i):
            self.lagrangian[i][j] += lag_lr * g_j.item()
        # print([l.cpu().numpy() for l in self.lagrangian])
        # print(g_i)
